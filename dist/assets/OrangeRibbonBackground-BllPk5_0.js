import{j as B}from"./framer-motion-CXhxFg8T.js";import{r as t}from"./react-vendor-Bxt1C2Z2.js";import{S as M,O as E,W as O,P as k,a as y,V as I,M as N}from"./three-vendor-DkVKcZRz.js";const H=()=>{const e=t.useRef(null),R=t.useRef(null),x=t.useRef(null),u=t.useRef(null),a=t.useRef(null),f=t.useRef(null),n=typeof window<"u"&&(window.innerWidth<=768||/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)),i=t.useRef(0),d=t.useRef(!0);return t.useEffect(()=>{if(!e.current)return;const v=new M;R.current=v;const m=new E(-1,1,1,-1,0,1);x.current=m;let c;try{c=new O({antialias:!n,alpha:!1,powerPreference:"high-performance"})}catch(r){console.warn("WebGL unavailable — using static background.",r);return}c.setSize(e.current.clientWidth,e.current.clientHeight),c.setPixelRatio(n?Math.min(window.devicePixelRatio,1):Math.min(window.devicePixelRatio,2)),e.current.appendChild(c.domElement);const l=c.domElement;u.current=c;const h=new k(2,2),g=new y({uniforms:{uTime:{value:0},uResolution:{value:new I(e.current.clientWidth,e.current.clientHeight)}},fragmentShader:`
      ...
      `});a.current=g;const S=new N(h,g);v.add(S);const p=1e3/(n?30:60);i.current=performance.now(),d.current=!0;let s=null;setTimeout(()=>{s=new IntersectionObserver(r=>{r.forEach(o=>{d.current=o.isIntersecting})},{threshold:.01,rootMargin:"50px"}),e.current&&s.observe(e.current)},100);const b=r=>{if(f.current=requestAnimationFrame(b),n){const o=r-i.current;if(o<p)return;i.current=r-o%p}else i.current=r;d.current&&(a.current&&(a.current.uniforms.uTime.value+=.01),c.render(v,m))};b(performance.now());const w=()=>{if(!e.current||!u.current||!a.current)return;const r=e.current.clientWidth,o=e.current.clientHeight;u.current.setSize(r,o),a.current.uniforms.uResolution.value.set(r,o)};return window.addEventListener("resize",w),()=>{s&&s.disconnect(),window.removeEventListener("resize",w),f.current&&cancelAnimationFrame(f.current),l&&l.parentNode&&l.parentNode.removeChild(l),c.dispose(),g.dispose(),h.dispose()}},[]),B.jsx("div",{ref:e,className:"orange-ribbon-background"})};export{H as default};
