import{j as S}from"./framer-motion-CXhxFg8T.js";import{r as t}from"./react-vendor-Bxt1C2Z2.js";import{S as B,O as M,W as O,P as E,a as I,V as k,M as y}from"./three-vendor-DkVKcZRz.js";const z=()=>{const e=t.useRef(null),w=t.useRef(null),R=t.useRef(null),c=t.useRef(null),n=t.useRef(null),u=t.useRef(null),l=typeof window<"u"&&(window.innerWidth<=768||/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)),a=t.useRef(0),f=t.useRef(!0);return t.useEffect(()=>{if(!e.current)return;const v=new B;w.current=v;const g=new M(-1,1,1,-1,0,1);R.current=g;const i=new O({antialias:!l,alpha:!1,powerPreference:"high-performance"});i.setSize(e.current.clientWidth,e.current.clientHeight),i.setPixelRatio(l?Math.min(window.devicePixelRatio,1):Math.min(window.devicePixelRatio,2)),e.current.appendChild(i.domElement),c.current=i;const d=new E(2,2),m=new I({uniforms:{uTime:{value:0},uResolution:{value:new k(e.current.clientWidth,e.current.clientHeight)}},fragmentShader:`
      ...
      `});n.current=m;const x=new y(d,m);v.add(x);const h=1e3/(l?30:60);a.current=performance.now(),f.current=!0;let s=null;setTimeout(()=>{s=new IntersectionObserver(r=>{r.forEach(o=>{f.current=o.isIntersecting})},{threshold:.01,rootMargin:"50px"}),e.current&&s.observe(e.current)},100);const p=r=>{if(u.current=requestAnimationFrame(p),l){const o=r-a.current;if(o<h)return;a.current=r-o%h}else a.current=r;!f.current&&r-a.current>1e3||(n.current&&(n.current.uniforms.uTime.value+=.01),i.render(v,g))};p(performance.now());const b=()=>{if(!e.current||!c.current||!n.current)return;const r=e.current.clientWidth,o=e.current.clientHeight;c.current.setSize(r,o),n.current.uniforms.uResolution.value.set(r,o)};return window.addEventListener("resize",b),()=>{s&&e.current&&s.unobserve(e.current),window.removeEventListener("resize",b),u.current&&cancelAnimationFrame(u.current),e.current&&c.current&&e.current.removeChild(c.current.domElement),c.current&&c.current.dispose(),n.current&&n.current.dispose(),d&&d.dispose()}},[]),S.jsx("div",{ref:e,className:"orange-ribbon-background"})};export{z as default};
